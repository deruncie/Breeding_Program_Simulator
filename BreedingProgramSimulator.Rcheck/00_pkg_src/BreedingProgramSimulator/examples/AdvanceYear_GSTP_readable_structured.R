...
run_send_rc_to_dh <- function(state, cfg) {
  ...
  state <- put_stage_pop(state = state, pop = dh_send, stage = cfg$dh_stage, source = src, ready_in_years = 2, stream = "main")
...
run_release_variety <- function(state, cfg) {
  ...
  state <- put_stage_pop(state = state, pop = variety, stage = "Variety", source = src, ready_in_years = 0, stream = "main")
